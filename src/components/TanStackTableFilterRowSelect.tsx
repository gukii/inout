import { useMemo, useRef, useState, useReducer, useEffect, InputHTMLAttributes, HTMLAttributes, HTMLProps } from 'react'
                      ...
                      <tr key={row.id} id={row.id}
                        onClick={ (e)=> {
                          if ((e.target as HTMLElement).tagName !== 'INPUT') {
                            // no alerts when clicking on checkBoxes
                            window.alert(`e.currentTarget.id:${e.currentTarget.id} ## row:${JSON.stringify(row)}`)
                          }
                        }}
                        className={`${ (idx+1) % 2 ? "bg-white text-gray-900" : "bg-gray-100 text-white-900"} border-b`}>
